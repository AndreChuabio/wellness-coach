...
def _build_context_block(health: dict, events: list[dict], recommendations: list, cal_summary: dict) -> str:
    """
...
def build_system_prompt(health: dict, events: list[dict]) -> dict:
    ...
    context_block = _build_context_block(
        health, events, recommendations, cal_summary)
...
Return only the greeting text, no quotes, no JSON wrapper."""
    ...
    if not os.getenv("ANTHROPIC_API_KEY"):
        ...
        return _fallback_context(health, events, recommendations, context_block)
...
def _fallback_context(health: dict, events: list[dict], recommendations: list, context_block: str) -> dict:
    """Fallback when no Anthropic API key is available."""
    greeting = (
        f"Good morning. I can see you got {health['sleep_hours']} hours of sleep last night "
